...
- (void)setupSubviews
{
    self.photoView = [[PhotoTweakView alloc] initWithFrame:self.view.bounds image:self.image maxRotationAngle:self.maxRotationAngle cropSize:self.cropSize];
...
- (void)saveBtnTapped
{
    ...
    NSLog(@"sourceSize = %@", NSStringFromCGSize(self.image.size));
    NSLog(@"outputWidth = %d", (int)self.image.size.width);
    NSLog(@"cropSize = %@", NSStringFromCGSize(self.photoView.cropView.frame.size));
    NSLog(@"imageViewSize = %@", NSStringFromCGSize(self.photoView.photoContentView.bounds.size));
