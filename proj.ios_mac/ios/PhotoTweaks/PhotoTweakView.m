...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        _upperLeft.hidden = _upperRight.hidden = _lowerLeft.hidden = _lowerRight.hidden = YES;
    }
    return self;
}
                        ...
                        cropSize:(CGSize) cropSize
{
    if (self = [super init]) {
        ...
        CGRect frame = CGRectMake(0, 0, cropSize.width, cropSize.height);
        ...
        _slider.center = CGPointMake(CGRectGetWidth(self.bounds) / 2, CGRectGetHeight(self.bounds) - 40);
